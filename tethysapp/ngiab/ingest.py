...
def publish(archive_path, run_name, *, job_id=None, progress=None):
    ...
    workspace = tempfile.mkdtemp(prefix="ngiab-ingest-")
    try:
...
def _publish_directory(source, run_name):
    """Put the prepared run where the picker will find it.

    Locally that is a move into the storage root, which is atomic within a filesystem and
    costs nothing. Hosted it is an object per file, which is why conversion runs first: a
    consolidated run is one parquet per schema group rather than one per catchment, so this
    uploads tens of objects where the unconverted run would have uploaded thousands.
    """
    ...
    for root, _dirs, files in os.walk(source):
        for name in files:
            ...
            key = posixpath.join(run_name, os.path.relpath(path, source).replace(os.sep, "/"))

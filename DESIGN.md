...
below are generated by `scripts/check-contrast.py`, which fails CI rather than trusting this
table: an earlier version listed four roles, omitted `--fg-subtle`, and claimed 5:1 for all of
them while `--fg-subtle` sat at 4.13:1 on the smallest text in the interface.

...
{% set CONDA_TETHYS_PATH = salt['environ.get']('CONDA_TETHYS_PATH') %}

Generate_Tethys_Settings_TethysCore:
  cmd.run:
    - name: >
        {{ CONDA_TETHYS_PATH }} settings
        --set DEBUG {{ DEBUG }}
...
Generate_ASGI_Service_TethysCore:
  cmd.run:
    - name: >
        {{ CONDA_TETHYS_PATH }} gen asgi_service
        --tethys-port {{ TETHYS_PORT }}
...
Create_Database_User_and_SuperUser_TethysCore:
  cmd.run:
    - name: >
        PGPASSWORD="{{ POSTGRES_PASSWORD }}" {{ CONDA_TETHYS_PATH }} db create
...
Create_Database_Portal_SuperUser_TethysCore:
  cmd.run:
    - name: >
        {{ CONDA_TETHYS_PATH }} db createsuperuser
...
Modify_Tethys_Site_TethysCore:
  cmd.run:
    - name: {{ CONDA_TETHYS_PATH }} site {{ TETHYS_SITE_CONTENT }}
    - unless: /bin/bash -c "[ -f "{{ TETHYS_PERSIST }}/setup_complete" ];"
{% endif %}
